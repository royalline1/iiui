...
class StudentFinancialReportSummaryReport(models.AbstractModel):
    ...
    def _get_report_values(self, docsid, data=None):
        ...
        if program_id:
            invoice = self.env['account.invoice'].search([('program_id', '=', program_id),('state', '=', 'paid')])
            program = self.env['odoocms.program'].search([('id', '=', program_id)])
            if invoice:
                for inv in invoice:
                    domain = [('invoice_id', '=', inv.id)]
                    line = {
                        "student_id":inv.student_id.id_number,
                        "student_name": inv.student_id.first_name+" "+inv.student_id.last_name,
                        "career": inv.program_id.career_id.name,
                        "program": inv.program_id.name,
                        "invoice_lines":[],
                    }
                    invoice_lines = self.env['account.invoice.line'].read_group(domain,['price_unit'],['fee_category_id'])

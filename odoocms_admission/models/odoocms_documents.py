...
class OdooCMSDocuments(models.Model):
    _name = 'odoocms.documents'
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('odoocms.documents') or _('New')
        res = super(OdooCMSDocuments, self).create(vals)
        return res
    ...
    application_ref = fields.Many2one('odoocms.application', invisible=1, copy=False)
...
class HrEmployeeAttachment(models.Model):
    ...
    doc_attach_rel = fields.Many2many('odoocms.documents', 'doc_attach_rel', 'attach_id', 'doc_id',

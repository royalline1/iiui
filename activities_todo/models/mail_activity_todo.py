...
class mail_activity_todo(models.TransientModel):
    ...
    current_activity_id = fields.Many2one("mail.activity", string="Current activity", compute=_compute_progress,)
    progress = fields.Float(string="Progress", compute=_compute_progress,)
    current_number = fields.Integer(string="Number")
    total_number = fields.Integer(string="Total")
    done_number = fields.Integer(string="Done", compute=_compute_progress, store=True,)
    res_reference = fields.Reference(selection='_selection_res_reference', string='Document',
        compute=_compute_res_reference, compute_sudo=True,)
    ...
    def start_todo(self):
        ...
        user = self.env.user
        domain = [("user_id", "=", user.id),('done', '!=', True)]
    ...
    def _check_and_update_activities(self):
        ...
        for act in activities:
            ...
            if act["state"] == "to_do" and self.env["mail.activity"].browse(act["activity_id"]).done:
                act["state"] = "done"
